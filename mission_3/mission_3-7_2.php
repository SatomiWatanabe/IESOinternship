...
//【投稿】$nameと$commentと$passwordが空でない＋hiddenの$editednumberが空である場合に処理を実行する
	...
	$sql = 'INSERT INTO birdchat (name, comment, time, password) VALUES (:name, :comment, :time, :password)';
...
//elseifはいくつでも指定できる。
	...
	foreach ($stmt as $re){
		...
		if ($re[number] == $editednumber and $re[password] == $password){
			...
			$sql = 'UPDATE birdchat SET name = :name, comment = :comment, time = :time WHERE number = :number';
	...
	//【削除】削除対象番号とパスワードが空でないときのみ処理を実行。
		...
		foreach ($stmt as $re){
			...
			//既存の投稿番号・パスワードと、フォームから受け取った削除対象番号・パスワードが一致した場合に処理
				...
				$sql = 'DELETE FROM birdchat WHERE number = :number';
