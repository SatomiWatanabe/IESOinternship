...
$data ='<>'.$name.'<>'.$comment.'<>'.$time.'<>'.$password;
...
//$nameと$commentと$passwordが空でない＋hiddenの$editednumberが空である場合に処理を実行する
	...
	fwrite($fp, $data."\n");
	/*開いたファイルに「<>名前<>コメント<>時間<>パスワード」を書き込む;
...
//$name, $comment, $password, $editiednumber(hidden)が空でない場合に処理を実行
	...
	//$arrayの要素数分だけループ処理
		...
		if ($ar[0] == $editednumber and $ar[4] == $password){
		//投稿番号がhiddenからの編集対象番号と一致＋パスワードが一致すれば、処理を実行。
			
			fwrite($fp, $editednumber.'<>'.$name.'<>'.$comment.'<>'.$time.'(編集済み)'.'<>'.$password."\n");
			//編集対象番号=投稿番号<>名前<>コメント<>時間（編集済み）<>パスワードを書き込む＋改行
	...
	//elseifはいくつでも指定できる。
		...
		//要素数分だけループ処理。(処理開始値;処理を繰り返す上限;変化式)
			...
			if ($ar[0] == $deletenumber and $ar[4] == $deletepassword){
			//投稿番号が削除対象番号と一致＋パスワードが一致すれば、処理を実行。
